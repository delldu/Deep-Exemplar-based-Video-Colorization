...
def compile():
    model, device = video_color.get_tvm_model()

    todos.data.mkdir("output")
    if not os.path.exists("output/video_color.so"):
        input1 = torch.randn(SO_B, SO_C, SO_H, SO_W)
        input2 = torch.randn(SO_B, SO_C, SO_H, SO_W)

        todos.tvmod.compile(model, device, [input1, input2], "output/video_color.so")
...
def predict(input_files, output_dir):
    ...
    tvm_model = todos.tvmod.load("output/video_color.so", str(device))
